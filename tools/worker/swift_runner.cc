...
std::string MaybeRewriteSwiftInterface(
  ...
  // Create the rewrite directory under cwd (rather than the system temp dir)
  // so the resulting path is relative to the Bazel sandbox. This keeps it
  // compatible with `SymlinkedInterfacePath`, which exits if asked to handle
  // an absolute path that lives outside `DEVELOPER_DIR`.
  auto tmpdir = TempDirectory::CreateIn(".rules_swift_iface_fix",
                                        "iface.XXXXXX");
  if (!tmpdir) {
    return interface_path;
  }
